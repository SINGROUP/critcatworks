...
def sort_two_objectives(a, b):
    return b[np.argsort(np.arange(b.shape[0]) + np.array([a.tolist().index(x) for x in b]))]
...
class CoverageLadderTask(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        reference_energy = fw_spec["temp"].get("reference_energy ", fw_spec["temp"].get("reference_energy", 0))
...
class GatherLadderTask(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        # workflow_id
        # branch_dct, property, calc_ids, ne_dct, analysis_ids, calc_parents, 
        # root_history, step_history, root_history, free_energy_correction, reference_energy
        # 
        ...
        workflows.update_one({'_id': fw_spec["workflow"]["_id"]}, {"$set": {"output" : wf_output}}, upsert=False) 
...
class AddRemoveLadderTask(FiretaskBase):
    ...
    def add_adsorbate(self, simulation, bond_length, db, 
        ...
        # b) proximity
        elif ranking_metric == "proximity":
            # already ordered
            ids = ids
        else:
            proximity_ids = ids.copy()
            desc = cluster.get_sites_descriptor(-1)
            remaining_desc = desc[ids]
            desc_ids = cluskit.cluster._rank_fps(remaining_desc, K=None, greedy=False)
            similarity_ids = ids[desc_ids]
            ids = sort_two_objectives(similarity_ids, proximity_ids)
    ...
    def remove_adsorbate(self, simulation, bond_length, db, 
        ...
        if ranking_metric == "similarity":
            ...
            ids = x2_to_x(adsorbate_atoms.get_positions(), bondlength = bond_length)
        else:
            similarity_ids = cluskit.cluster._rank_fps(desc, K=None, greedy=False)
            proximity_ids = x2_to_x(adsorbate_atoms.get_positions(), bondlength = bond_length)
            ids = sort_two_objectives(similarity_ids, proximity_ids)
