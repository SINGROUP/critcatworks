...
class NCReadTask(FiretaskBase):
    ...
    Args:
        ...
        cell (nd numpy array) : consistent cell size as a 3 x 3 matrix
    ...
    optional_params = ['cell_factor', 'cell']

    def run_task(self, fw_spec):
        path = self["path"]
        cell_factor = self.get("cell_factor", None)
        cell_factor = self.get("cell", [[25.0, 0.0, 0.0],[0.0, 25.0, 0.0],[0.0, 0.0, 25.0]])

        logging.debug(fw_spec)

        structures = read_structures_locally(path, cell_factor = cell_factor, cell = cell)
...
def read_structures(path, spec = {}, cell_factor = None, cell = [[25.0, 0.0, 0.0],[0.0, 25.0, 0.0],[0.0, 0.0, 25.0]]):
    """
    ...
    """
    firetask1  = NCReadTask(path=path, cell_factor = cell_factor, cell = cell)
...
def read_structures_locally(path, cell_factor = None, cell = [[25.0, 0.0, 0.0],[0.0, 25.0, 0.0],[0.0, 0.0, 25.0]]):
    """
